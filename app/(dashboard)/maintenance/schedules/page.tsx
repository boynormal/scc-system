  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">ตารางงานซ่อมบำรุง</h1>
          <p className="text-muted-foreground text-sm mt-1">กำหนดการทั้งหมด {schedules.length} รายการ</p>
        </div>
        <Link
          href="/api/cron/generate-schedules"
          className="flex items-center gap-2 px-4 py-2 border border-border text-muted-foreground text-sm rounded-lg hover:bg-muted/60"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "ทั้งหมด", value: schedules.length, color: "bg-muted0" },
          ...
          <div key={c.label} className="bg-card border border-border rounded-xl p-4 shadow-sm">
            ...
            <p className={`text-2xl font-bold mt-1 ${c.color === "bg-red-500" && overdue > 0 ? "text-red-600" : "text-foreground"}`}>{c.value}</p>
          </div>
        ))}
      </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted border-b border-border">
                <tr>
                  {["กำหนดการ", "เครื่องจักร", "ประเภท", "แผน", "ใบสั่งงาน", "สถานะ", ""].map((h, i) => (
                    <th key={i} className="px-5 py-3 text-left font-semibold text-muted-foreground text-xs uppercase tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {schedules.map(schedule => {
                  ...
                  return (
                    <tr key={schedule.id} className={`hover:bg-muted/60 transition-colors ${isOverdue ? "bg-red-50/30" : ""}`}>
                      ...
                      <td className="px-5 py-3.5">
                        <Link href={`/machines/${schedule.machine.id}`} className="hover:text-blue-600 transition-colors">
                          <p className="font-medium text-foreground">{schedule.machine.name}</p>
                          <p className="text-xs text-muted-foreground">{schedule.machine.code} · {schedule.machine.branch.name}</p>
                        </Link>
                      </td>
                      <td className="px-5 py-3.5">
                        <span
                      ...
                      <td className="px-5 py-3.5">
                        <Link href={`/maintenance/plans/${schedule.plan.id}`} className="text-foreground hover:text-blue-600 text-sm">
                          {schedule.plan.name}
                        </Link>
                      </td>
                      <td className="px-5 py-3.5">
                        {schedule.workOrders[0] ? (
                          ...
                          <span className="text-muted-foreground text-xs">ยังไม่มี</span>
                        )}
                      </td>
                      <td className="px-5 py-3.5">
