...
import { costCenterSchema, createCostCenter, listCostCenters } from "@/modules/finance"

export const GET = withAuth(async (req, _ctx, session) => {
  const denied = forbidUnlessPermission(session.user.roles as UserRole[], "expenses", "read")
  if (denied) return denied
  const { searchParams } = new URL(req.url)
  const result = await listCostCenters(prisma, {
...
export const POST = withAuth(async (req, _ctx, session) => {
  ...
  const result = await createCostCenter(prisma, {
    companyId: session.user.companyId as string,
    roles: session.user.roles as UserRole[],
    input: parsed.data,
  })
