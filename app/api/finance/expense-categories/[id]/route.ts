...
import { deleteExpenseCategory, expenseCategorySchema, updateExpenseCategory } from "@/modules/finance"
...
export const PATCH = withAuth(async (req, ctx, session) => {
  ...
  const result = await updateExpenseCategory(prisma, {
...
export const DELETE = withAuth(async (_req, ctx, session) => {
  ...
  if (denied) return denied
  const { id } = await ctx.params
  const result = await deleteExpenseCategory(prisma, {
    companyId: session.user.companyId as string,
    roles: session.user.roles as UserRole[],
    id,
  })
