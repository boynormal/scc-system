...
import { deleteProcess, processSchema, updateProcess } from "@/modules/finance"
...
export const PATCH = withAuth(async (req, ctx, session) => {
  ...
  const parsed = processSchema.partial().safeParse(body)
  if (!parsed.success) throw new ValidationError("ข้อมูลไม่ถูกต้อง")
  const result = await updateProcess(prisma, {
...
export const DELETE = withAuth(async (_req, ctx, session) => {
  ...
  if (denied) return denied
  const { id } = await ctx.params
  const result = await deleteProcess(prisma, {
    companyId: session.user.companyId as string,
    roles: session.user.roles as UserRole[],
    id,
  })
