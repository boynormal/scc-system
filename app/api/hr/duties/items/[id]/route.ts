...
import { deleteDutyItem, updateDutyItem, updateDutyItemSchema } from "@/modules/hr"
...
export const PATCH = withAuth<Ctx>(async (req, ctx, session) => {
  ...
  const parsed = updateDutyItemSchema.safeParse(body ?? {})
  if (!parsed.success) throw new ValidationError("ข้อมูลรายการไม่ถูกต้อง")
  const result = await updateDutyItem(prisma, {
...
export const DELETE = withAuth<Ctx>(async (req, ctx, session) => {
  const { id } = await ctx.params
  const result = await deleteDutyItem(prisma, {
    companyId: session.user.companyId as string,
    roles: session.user.roles as UserRole[],
    userId: session.user.id as string,
    id,
