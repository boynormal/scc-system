import { prisma } from "@/shared/db"
import { withAuth } from "@/lib/api-handler"
import { ValidationError } from "@/lib/errors"
import { listJobs, createJob, createJobSchema } from "@/modules/transport"
import type { TransportJobStatus, TransportJobPriority } from "@prisma/client"

export const GET = withAuth(async (req, _ctx, session) => {
  const { searchParams } = new URL(req.url)
  const branchId = searchParams.get("branchId")
  const status = searchParams.get("status") as TransportJobStatus | null
  const priority = searchParams.get("priority") as TransportJobPriority | null
  const search = searchParams.get("search")
  const page = Number(searchParams.get("page") ?? "1")
  const pageSize = Number(searchParams.get("pageSize") ?? "20")

  const data = await listJobs(prisma, {
    companyId: session.user.companyId as string,
    roles: session.user.roles as never,
    branchId,
    status,
    priority,
    search,
    page,
    pageSize,
  })
  return Response.json({ data })
})

export const POST = withAuth(async (req, _ctx, session) => {
  const body = await req.json()
  const parsed = createJobSchema.safeParse(body)
  if (!parsed.success) throw new ValidationError(JSON.stringify(parsed.error.flatten()))

  const data = await createJob(prisma, {
    companyId: session.user.companyId as string,
    userId: session.user.id as string,
    roles: session.user.roles as never,
    input: parsed.data,
  })
  return Response.json({ data }, { status: 201 })
})
