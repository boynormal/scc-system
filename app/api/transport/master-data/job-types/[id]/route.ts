...
import { deactivateJobType, updateJobType, updateJobTypeSchema } from "@/modules/transport"
...
export const PATCH = withAuth(async (req, ctx, session) => {
  ...
  const data = await updateJobType(prisma, {
    id,
...
export const DELETE = withAuth(async (_req, ctx, session) => {
  const { id } = await ctx.params
  const data = await deactivateJobType(prisma, {
    id,
    companyId: session.user.companyId as string,
    roles: session.user.roles as never,
  })
