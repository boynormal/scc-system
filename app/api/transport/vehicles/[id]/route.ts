...
import { getVehicleById, updateVehicle, updateVehicleSchema, deleteVehicle } from "@/modules/transport"

type Ctx = { params: Promise<{ id: string }> }

export const GET = withAuth<Ctx>(async (_req, ctx, session) => {
  const { id } = await ctx.params
  const data = await getVehicleById(prisma, {
    id,
...
export const PUT = withAuth<Ctx>(async (req, ctx, session) => {
  ...
  const data = await updateVehicle(prisma, {
    id,
    companyId: session.user.companyId as string,
    roles: session.user.roles as never,
    input: parsed.data,
